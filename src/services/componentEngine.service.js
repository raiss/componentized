  ...
  return appObj.map(comp => {
    ...
    switch (type) {
      case GENERIC_COMPONENT:
        return (<GenericComponent key={comp.id} validator={validator} validate={validateInput} componentObj={comp}/>);
        break;
      case SECTION:
        return (<Section key={comp.id} title={comp.title}>{createComponents(comp.children, validator)}</Section>);
        break;
      default:
